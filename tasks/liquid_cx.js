...
module.exports = function (grunt) {
    ...
    /*
     * Pull in custom liquid-ext file
     */
    ...
    var registerFilters = function(options) {

        /**
         * Register translation filter
         */
        Liquid.Template.registerFilter({"t": function (input) {
            if ( options.language[input] !== undefined ) {
                return options.language[input];
            }
            return input;
        }});
    };

    grunt.registerMultiTask('liquid_cx', 'Compile liquid templates.', function () {
        var done = this.async();
